...
function playGame() {
  ...
  // 20% chance of losing all points
  if (randomNumber < 0.3) {
    score = 0;
    result = 'You lost all your points!';
    showImage('Rotten.PNG');
  } else {
    // Randomly determine if can is fresh or rotten
    const isFresh = Math.random() < 1;
    if (isFresh) {
      score++;
      result = 'You got a fresh can!';
         showImage('Fresh.PNG');
    } else {
      result = 'You got a rotten can!';
      showImage('Rotten.PNG');
    }
  }
...
// //   fill (0,0,204);
// //   text('SPAM GAME', width / 3, height / 10);
...
//   // Draw the SPAM
//   image(spam.img, spam.x*2 - spam.w/2, spam.y - spam.h/2, spam.w, spam.h);
